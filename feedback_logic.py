...
system_prompt_tail = """
1~6학년 수학 학습 요소는 덧셈, 뺄셈, 곱셈, 짝수, 홀수, 사칙연산, 등호, 부등호, 삼각형,사각형,원,꼭짓점,변,오각형,육각형, 시,분,약, cm, m, 표,그래프, 나눗셈,몫,나머지,나누어떨어진다,분수,분모,분자,단위분수,진분수,가분수,대분수,자연수,소수,소수점(.),/, 직선,선분,반직선,각, (각의)꼭짓점, (각의)변,직각,예각,둔각,수직,수선,평행,평행선,원의 중심,반지름,지름,이등변삼각형,정삼각형,직각삼각형,예각삼각형,둔각삼각형,직사각형,정사각형,사다리꼴,평행사변형,마름모,다각형,정다각형,대각선, 초,각도 단위의 도, mm, km, L, mL, g, kg, t, 그림그래프,막대그래프,꺾은선그래프, 약수,공약수,최대공약수,배수,공배수,최소공배수,약분,통분,기약분수, 합동,대칭,대응점,대응변,대응각,선대칭도형,점대칭도형,대칭축,대칭의 중심,직육면체,정육면체,면,모서리,밑면,옆면,겨냥도,전개도,각기둥,각뿔,원기둥,원뿔,구,모선, 이상,이하,초과,미만,올림,버림,반올림,가로,세로,밑변,높이,원주,원주율, 제곱cm, 제곱m, 제곱km, 세제곱cm, 세제곱m, 비,기준량,비교하는 양,비율,백분율,비례식,비례배분, : , %, 평균,띠그래프,원그래프,가능성입니다.
...
def generate_feedback(problem):
    ...
    try:
        ...
        system_prompt = system_prompt_head + system_prompt_tail
